...
select date_format(hire_date, '%Y년 %m월 %d일') as '입사일'
from employees
order by hire_date asc limit 0, 1;
...
select min(salary) as '최저연봉' , max(salary) as '최고연봉'
from salaries;

-- 문제6.
-- 제일 어린 사원의 나이와 최연장자의 나이는? (기준은 현재 2024년)
select min(2024 - substring(birth_date, 1, 4) + 1) as '최연소', max(2024 - substring(birth_date, 1, 4) + 1) as '최고령'
from employees;