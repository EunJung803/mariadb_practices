-- 1) 집계쿼리 : select절에 통계함수(count, max, min, sum, avg, variance, stddev, ...)
select avg(salary), sum(salary)
from salaries;

-- 2) select 절에 집계함수(그룹함수, 통계함수)가 있는 경우, 어떤 칼럼도 select절에 올 수 없다!!!!
-- emp_no는 아무 의미가 없다.
-- 오류!!
...
-- 4) group by에 참여하고 있는 칼럼은 projection이 가능하다.
-- 예제) 사원별 평균 월급은?
select emp_no, avg(salary)
from salaries
group by emp_no;
